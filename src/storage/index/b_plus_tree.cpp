...
N *BPLUSTREE_TYPE::Split(N *node) {
  ...
  if(node->IsLeafPage()){
    ...
    for(int i = 0;i < lpage->GetSize();i++){
      auto temp = lpage->GetItem(i);
      std::cout << "In lpage, the key is " << temp.first << " value is " << temp.second << std::endl;
    }
    ...
    for(int i = 0;i < new_lpage->GetSize();i++){
      auto temp = new_lpage->GetItem(i);
      std::cout << "In new lpage, the key is " << temp.first << " value is " << temp.second << std::endl;
    }
  } else {
