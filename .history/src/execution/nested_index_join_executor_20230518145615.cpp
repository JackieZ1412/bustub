...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
  Transaction *txn = Gexec_ctx_->GetTransaction();
  Tuple left_tuple{};
  Tuple right_tuple{};
  RID lrid{};
  std::vector<Value> values;
  std::vector<RID> right_rids;
  while(child_->Next(&left_tuple,&lrid)){
    auto v = plan_->KeyPredicate()->Evaluate(&left_tuple, child_->GetOutputSchema());
    tree_->ScanKey(Tuple{{v}, index_info_->index_->GetKeySchema()}, &right_rids, exec_ctx_->GetTransaction());
    if(!right_rids.empty()){
      table_info_->table_->GetTuple(right_rids[0],&right_tuple, exec_ctx_->GetTransaction());
      *tuple = InnerJoin(&left_tuple,&right_tuple);
      *rid = tuple->GetRid();
      return true;
    }
    if(plan_->GetJoinType() == JoinType::LEFT){
      *tuple = NULLLeftJoin(&left_tuple);
      *rid = tuple->GetRid();
      return true;
    }
  }
