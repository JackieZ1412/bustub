...
auto NestIndexJoinExecutor::NULLLeftJoin(Tuple *tuple) -> Tuple{
  ...
  auto length = child_->GetOutputSchema().GetColumnCount();
  for(decltype(length) i = 0; i < length; i++){
    values.emplace_back(tuple->GetValue(&lchild_->GetOutputSchema(), i));
  }
  for(decltype(length) i = 0; i < length; i++){
    values.emplace_back(ValueFactory::GetNullValueByType(plan_-InnerTableSchema().OutputSchema().GetColumn(i).GetType()));
  }
