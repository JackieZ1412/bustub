    ...
    if(free_list_.empty() == false){
      *page_id = AllocatePage();
      ...
      if(is_allpined){
        ...
        replacer_->Victim(old_frame_id); // get a free frame_id of a frame
        if(pages_[*old_frame_id].IsDirty() == true){
          page_id_t old_page_id;
          for(auto it = page_table_.begin();it != page_table_.end(); it++){
            if((*it).second == *old_frame_id){
              old_page_id = (*it).first;
              page_table_.erase(it);
              break;
            }
          }
          disk_manager_->WritePage(old_page_id,pages_[*old_frame_id].GetData());
        }
        page_table_.emplace(page_id,*old_frame_id);
        pages_[*old_frame_id].page_id_ = *page_id;
        pages_[*old_frame_id].is_dirty_ = false;
        pages_[*old_frame_id].pin_count_ = 0;
      }
    }
