...
auto NestedLoopJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
  ...
  while(true){
    if(iter_ == right_tuples_.end()){
      if(plan_->GetJoinType() == JoinType::LEFT && !right_matched_){
        // LOG_DEBUG("left and right not exist matched and need null left join");
    ...
    while(iter_ != right_tuples_.end()){
      if(Matched(&left_tuple_,&(*iter_))){
       // LOG_DEBUG("left and right are matched");
