...
class LockManager {
 public:
  ...
  auto IsHigherLevel(LockMode lm1, LockMode lm2) -> bool;

  void DeleteTableLockFromSet(Transaction *txn, std::shared_ptr<LockRequest> req);

  void InsertTableLockFromSet(Transaction *txn, std::shared_ptr<LockRequest> req);

  void DeleteRowLockFromSet(Transaction *txn, std::shared_ptr<LockRequest> req);

  void InsertRowLockFromSet(Transaction *txn, std::shared_ptr<LockRequest> req);

  auto GrantLock(std::shared_ptr<LockRequestQueue> lrq, std::shared_ptr<LockRequest> req) -> bool;
